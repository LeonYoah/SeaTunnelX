...
# SeaTunnel Capability Proxy

`seatunnel-capability-proxy` is a lightweight sidecar service for platform integration. It does
...
- [API.md](/Users/mac/Documents/projects/seatunnel/tools/seatunnel-capability-proxy/API.md)
- [TESTING.md](/Users/mac/Documents/projects/seatunnel/tools/seatunnel-capability-proxy/TESTING.md)
...
```bash
./mvnw -f tools/seatunnel-capability-proxy/pom.xml spotless:apply
./mvnw -f tools/seatunnel-capability-proxy/pom.xml test
./mvnw -f tools/seatunnel-capability-proxy/pom.xml -DskipTests verify
```

Default output is a thin jar:

- `target/seatunnel-capability-proxy-3.0.0-SNAPSHOT-2.12.15.jar`
...
```bash
./mvnw -f tools/seatunnel-capability-proxy/pom.xml -Pstandalone-bin -DskipTests verify
```
...
- [seatunnel-capability-proxy.sh](/Users/mac/Documents/projects/seatunnel/tools/seatunnel-capability-proxy/bin/seatunnel-capability-proxy.sh)

Typical deployment:

1. Copy the thin proxy jar to the installed SeaTunnel home, for example:
   - `${SEATUNNEL_HOME}/tools/seatunnel-capability-proxy.jar`
2. Copy the startup script to:
   - `${SEATUNNEL_HOME}/bin/seatunnel-capability-proxy.sh`
3. Start the proxy with the installed SeaTunnel runtime:

```bash
export SEATUNNEL_HOME=/opt/seatunnel
export SEATUNNEL_PROXY_JAR=${SEATUNNEL_HOME}/tools/seatunnel-capability-proxy.jar
export JAVA_HOME=/path/to/jdk8-or-jdk11

sh ${SEATUNNEL_HOME}/bin/seatunnel-capability-proxy.sh \
...
7. Start the proxy
   - `sh ${SEATUNNEL_HOME}/bin/seatunnel-capability-proxy.sh -Dseatunnel.capability.proxy.port=18080`
8. Run smoke checks
   - `GET /healthz`
