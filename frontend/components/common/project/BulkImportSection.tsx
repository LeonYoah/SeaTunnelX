...
export function BulkImportSection({
  ...
  const emptyStateText = isEditMode
    ? '暂无待添加内容，请在上方导入'
    : '暂无分发内容，请在上方导入';
  ...
  return (
    <>
          ...
          <div className='flex items-start gap-3'>
            ...
            <div className='flex-1'>
              ...
              <p className='text-sm text-blue-700 dark:text-blue-300 mt-1'>
                ...
                <span className='font-medium'>{totalExistingItems}</span>{' '}
                个分发内容。
              </p>
            </div>
          </div>
      ...
      <Dialog open={fileUploadOpen} onOpenChange={onFileUploadOpenChange}>
        <DialogContent
          className={`${isMobile ? 'max-w-[90vw] max-h-[80vh]' : 'max-w-lg'}`}
        >
          <DialogHeader>
            <DialogTitle>文件导入</DialogTitle>
            <DialogDescription className='text-xs'>
              支持 .txt 和 .jsonl 格式 • 每行一个内容 • 空行自动忽略 •
              大小限制：5MB
            </DialogDescription>
          </DialogHeader>
          ...
          <div className='space-y-3'>
            <div className='p-3 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-lg'>
              ...
              <div className='text-xs text-amber-700 dark:text-amber-300 space-y-1'>
                ...
                <div>
                  文件大小:{' '}
                  {pendingFile ? (pendingFile.file.size / 1024).toFixed(1) : 0}{' '}
                  KB
                </div>
              </div>
            </div>
