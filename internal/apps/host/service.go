...
func (s *Service) UpdateAgentStatus(ctx context.Context, ipAddress string, agentID string, version string, systemInfo *SystemInfo) (*Host, error) {
...
// GetInstallScript returns the full installation script content.
// GetInstallScript 返回完整的安装脚本内容。
func (s *Service) GetInstallScript() string {
	return fmt.Sprintf(`#!/bin/bash
# SeaTunnelX Agent Installation Script
# SeaTunnelX Agent 安装脚本
# Auto-generated by Control Plane
# 由 Control Plane 自动生成
...
download_agent() {
    ...
    curl -sSL "http://${CONTROL_PLANE_ADDR}/api/v1/agent/download?os=${OS}&arch=${ARCH}" -o /tmp/seatunnelx-agent
    chmod +x /tmp/seatunnelx-agent
}
...
install_agent() {
    ...
    sudo mv /tmp/seatunnelx-agent ${INSTALL_DIR}/seatunnelx-agent
    
    echo "Creating config directory..."
    sudo mkdir -p ${CONFIG_DIR}
    
    echo "Generating config file..."
    sudo cat > ${CONFIG_DIR}/config.yaml << EOF
...
log:
  level: "info"
  output: "/var/log/seatunnelx-agent/agent.log"
EOF
}
...
[Service]
Type=simple
ExecStart=${INSTALL_DIR}/seatunnelx-agent --config ${CONFIG_DIR}/config.yaml
Restart=always
RestartSec=5
User=root
...
start_agent() {
    ...
    if sudo systemctl is-active --quiet ${SERVICE_NAME}; then
        ...
        echo "Error: Agent failed to start. Check logs with: journalctl -u ${SERVICE_NAME}"
        exit 1
    fi
}
...
main() {
    echo "=========================================="
    echo "SeaTunnelX Agent Installation"
    echo "=========================================="
    ...
    echo "Agent is now running and connected to Control Plane."
    echo "=========================================="
}
